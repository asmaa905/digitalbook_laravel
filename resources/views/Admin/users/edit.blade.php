...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if($user->image)
                    <div class="mb-2">
                        ...
                        <a href="{{ asset('storage/' . $user->image) }}" target="_blank" class="ms-2">View Current</a>
                    </div>
                @endif
            ...
            <div id="publisher-fields" style="display: {{ $user->role === 'Publisher' ? 'block' : 'none' }};">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="job_title" name="job_title" 
                                   value="{{ old('job_title', $user->publisher->job_title ?? '') }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="publishing_house_id" name="publishing_house_id">
                                ...
                                @foreach($publishingHouses as $house)
                                    <option value="{{ $house->id }}" 
                                        {{ old('publishing_house_id', $user->publisher->publishing_house_id ?? '') == $house->id ? 'selected' : '' }}>
                                        {{ $house->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-3">
                    ...
                    @if($user->publisher && $user->publisher->identity)
                        <div class="mb-2">
                            <a href="{{ asset('storage/' . $user->publisher->identity) }}" target="_blank" class="btn btn-sm btn-outline-primary">
