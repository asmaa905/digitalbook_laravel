                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="mb-4">
                                ...
                                <select id="publish_house_id" name="publish_house_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    <option value="{{ $house->id }}" {{ old('publish_house_id', $book->publish_house_id) == $house->id ? 'selected' : '' }}>{{ $house->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select id="user_id" name="user_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    <option value="{{ $publisher->id }}" {{ old('user_id', $book->user_id) == $publisher->id ? 'selected' : '' }}>{{ $publisher->name }}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="mb-2">
                                    <img src="{{ asset('storage/'.$book->cover_image) }}" alt="{{ $book->title }}" class="h-20 w-20 object-cover rounded">
                                </div>
                                <x-input id="cover_image" class="block mt-1 w-full" type="file" name="cover_image" />
                                @error('cover_image')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                @if($book->ebook_file)
                                <div class="mb-2">
                                    <span class="text-sm text-gray-500">Current file: {{ basename($book->ebook_file) }}</span>
                                </div>
                                @endif
                            ...
                            <div class="mb-4">
                                ...
                                @if($book->audio_file)
                                <div class="mb-2">
                                    <span class="text-sm text-gray-500">Current file: {{ basename($book->audio_file) }}</span>
                                </div>
                                @endif
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="5" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('description', $book->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
